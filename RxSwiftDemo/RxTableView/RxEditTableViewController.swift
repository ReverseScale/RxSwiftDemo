//
//  RxEditTableViewController.swift
//  RxSwiftDemo
//
//  Created by WhatsXie on 2018/4/9.
...
class RxEditTableViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        Observable.of(refreshCommand, addCommand, movedCommand, deleteCommand)
            ...
            .share(replay: 1)
            .bind(to: tableView.rx.items(dataSource: RxEditTableViewController.dataSource()))
            .disposed(by: disposeBag)
    }

    func createItem() {
        refreshButton = UIBarButtonItem(title: "refresh", style: UIBarButtonItemStyle.plain, target: self, action: nil)
        addButton = UIBarButtonItem(title: "add", style: UIBarButtonItemStyle.plain, target: self, action: nil)
...
extension RxEditTableViewController {
    //创建表格数据源
